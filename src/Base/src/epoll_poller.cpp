 ...
 * @LastEditors: lqf
 * @LastEditTime: 2022-03-10 20:25:57
 * @Description: 
 */
...
namespace Itachi
{
    static const int EPOLL_WAIT_TIMEOUT = 100000;
    ...
    void EpollPoller::fillActivateChannles(ActivateChannels &activateChannels, int count)
    {
        for (const auto &val : m_epollRs)
        {
            ...
            // {
             ptr->setRevent(val.events);
            // }
            activateChannels.push_back(ptr);
            if (--count == 0)
            {
                break;
            }
    ...
    void EpollPoller::removeChannel(Channel *channel)
    {
        ...
        LOG_DEBUG<<"channel is removed   fd is "<<channel->getFd();
